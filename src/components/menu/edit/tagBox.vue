...
export default {
  ...
  computed: {
    commandDisabled() {
      ...
      return (
        minder.queryCommandState && minder.queryCommandState("resource") === -1
      );
    }
  },
  mounted() {
  ...
  methods: {
    ...
    //兼容用例和手工任务：用例使用自带的原editResource方法，手工引入editResourceOuter
    editResource_fe(resourceName) {
      if (this.isHandleFlag) {
        this.editResourceOuter(resourceName);
      } else {
        this.editResource(resourceName);
      }
    },
