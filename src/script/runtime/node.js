define(function (require, exports, module) {
  function NodeRuntime() {
    ...
    main.button({
      ...
      enable: function () {
        // zll 添加，只有模块和用例才可以收起  //没有模块和用例标签的纯文件夹仍支持
        let node = minder.getSelectedNode();
        if (node&&node.data && node.data.resource&&node.data.resource.length>0) {
          let resourceName = node.data.resource[0]
          if (resourceName !== "用例" && resourceName !== '模块') {
            return false
          }
        }
